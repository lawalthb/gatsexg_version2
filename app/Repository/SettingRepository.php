...
class SettingRepository
{
    ...
    // save referral setting
    public function saveOthersApiSetting($request)
    {
        $response = ['success' => false, 'message' => __('Invalid request')];
        DB::beginTransaction();
        try {
            if (isset($request->cryptoCompare)) {
                AdminSetting::updateOrCreate(['slug' => 'cryptoCompare'], ['value' => $request->cryptoCompare]);
            }
            $response = [
                'success' => true,
                'message' => __('Others API updated successfully')
            ];
        } catch (\Exception $e) {
            DB::rollBack();
            $response = [
    ...
    public function userNavberSettingSave($request)
    {
        try {
            ...
            $response = [
                'success' => true,
                'message' => __('User navbar setting updated successfully')
            ];
        } catch (\Exception $e) {
            $response = [
                'success' => false,
            ...
            storeException('userNavberSettingSave',$e->getMessage());
        }
        return $response;
    }
