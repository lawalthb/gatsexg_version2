...
//        $maxReferralLevel = 3;
    ...
    // store data to affiliation history for trade
    public function storeTradeAffiliationHistory($transaction, $userId)
    {
        if ($transaction != null) {
            $adminSettings = $this->checkAdminSettings();
            $tradeUser = $userId;
            ...
            $coinType = $transaction->coin_type;
            $maxReferralLevel = max_level();
            $this->logger->log('storeTradeAffiliationHistory for user id', $userId);
            try {
                $userAffiliation = $this->parentReferrals($maxReferralLevel, $tradeUser);
                if (!empty($userAffiliation)) {
                    $this->calculateReferralFeesForTrade($adminSettings, $transactionId, $userAffiliation, $transaction->fees,  $maxReferralLevel, $coinType);
                }
            } catch (\Exception $e) {
                $this->logger->log('storeTradeAffiliationHistory', $e->getMessage());
            }
            $this->logger->log('storeTradeAffiliationHistory tx id', $transactionId);
            $this->logger->log('storeTradeAffiliationHistory', 'distributing trade affiliation bonus ends');
        }
    ...
    // calculate referral fees
    protected function calculateReferralFeesForTrade($adminSettings, $transactionId, $affiliateUsers, $systemFees, $maxReferralLevel = 1, $coinType)
    {
        $affiliationHistoryData['system_fees'] = $systemFees;
        $affiliationHistoryData['child_id'] = $affiliateUsers->user_id;
        $affiliationHistoryData['status'] = STATUS_ACTIVE;
        $affiliationHistoryData['transaction_id'] = $transactionId;
        $affiliationHistoryData['order_type'] = 2;
        $affiliationHistoryData['coin_type'] = $coinType;
        ...
        for ($i = 1; $i <= $maxReferralLevel; $i++) {
            ...
            if ($affiliateUsers->{$parent_level}) {
                try {
                    $this->logger->log('calculateReferralFeesForTrade', 'started');
                    $affiliationHistoryData['user_id'] = $affiliateUsers->{$parent_level};
                    $this->logger->log('user id', $affiliationHistoryData['user_id']);
                    $fees_percent = isset($adminSettings[$fees_level]) ? $adminSettings[$fees_level] : '0';
                    $this->logger->log('$fees_percent', $fees_percent);
                    $affiliationHistoryData['amount'] = ($systemFees * $fees_percent) / 100;
                    $affiliationHistoryData['level'] = $i;
                    $this->logger->log('system fees', $systemFees);
                    $this->logger->log('$fees_ amount', $affiliationHistoryData['amount']);
                    $this->logger->log('level', $affiliationHistoryData['level']);
                    $userWallet = get_primary_wallet($affiliationHistoryData['user_id'], $coinType);
                    if (isset($userWallet)) {
                        $affiliationHistoryData['wallet_id'] = $userWallet->id;
                        $this->logger->log('calculateReferralFeesForTrade balance increment wallet ', $userWallet->id);
                        $this->logger->log('wallet balance before ', $userWallet->balance);
                        $userWallet->increment('balance', $affiliationHistoryData['amount']);
                        $this->logger->log('wallet balance after ', $userWallet->balance);

                        $coin = Coin::where(['type' => $coinType])->first();
                        $this->logger->log('coin fees decrement', ' referral fees ');
                        $this->logger->log('trade id', $transactionId);
                        $this->logger->log('coin total fees', custom_number_format($coin->total_fees));
                        $this->logger->log('trade referral fees and should decrement in coin total fees', $affiliationHistoryData['amount']);
                        $coin->decrement('total_fees', $affiliationHistoryData['amount']);
                        $this->logger->log('after fees decrement', custom_number_format($coin->total_fees));
                    }
                    AffiliationHistory::create($affiliationHistoryData);
                } catch (\Exception $e) {
                    return false;
