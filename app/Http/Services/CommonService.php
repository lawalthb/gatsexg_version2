...
//seller ranking points add in current ranking points table
    ...
    public function sendNotificationToUser($user_id,$title,$message)
    {
        try {
            ...
            $channel = 'usernotification_'.$user_id;
            $config = config('broadcasting.connections.pusher');
            $pusher = new Pusher($config['key'], $config['secret'], $config['app_id'], $config['options']);
    ...
    // save payment method
    public function paymentMethodSaveProcess($request)
    {
        $response = [
            'success' => false,
        ...
        $old_img = '';
        if($request->edit_id) {
            $item = PaymentMethod::find($request->edit_id);
            if(isset($item) && (!empty($item->image))) {
                $old_img_file = $item->image;
                $old_imgs = explode('/',$old_img_file);
                $old_img = $old_imgs['5'];
            }
        }
        ...
        try {
            if ($request->edit_id) {
                if(isset($item)) {
                    ...
                    if($update) {
                        CountryPaymentMethod::where('payment_method_id',$item->id)->delete();
                        if (isset($request->country[0])) {
                            $this->save_country_payment_method($request->country,$item->id);
                        }
                        $response = [
                            'success' => true,
                ...
                if($save) {
                    if (isset($request->country[0])) {
                       $this->save_country_payment_method($request->country,$save->id);
                    }
                    $response = [
                        'success' => true,
    ...
    public function save_country_payment_method($countries, $payment_id)
    {
        foreach ($countries as $country) {
            CountryPaymentMethod::create(['payment_method_id' => $payment_id, 'country' => $country]);
        }
    }

    // user buy report
    public function authUserTrade($start_date, $end_date, $type)
    {
        $data = Order::where(['status' => TRADE_STATUS_TRANSFER_DONE, 'type' => $type])
            ->where(function ($query)  {
                $query->where('buyer_id', Auth::id())
                    ->orWhere('seller_id', Auth::id());
            })->select(
                DB::raw('monthname(orders.created_at) as month'),
                DB::raw('SUM(orders.amount) as balance'))
