...
//        $messages=[];
    ...
    public function adminSaveOthersApiSettings(Request $request)
    {
        $rules = [
            'cryptoCompare' => 'required'
        ];
        $validator = Validator::make($request->all(), $rules);
        if ($validator->fails()) {
            $errors = [];
            ...
            $data['message'] = $errors;
            return redirect()->route('adminSettings', ['tab' => 'others_api'])->with(['dismiss' => $errors[0]]);
        }
        try {
            if ($request->post()) {
                $response = $this->settingRepo->saveOthersApiSetting($request);
                if ($response['success'] == true) {
                    return redirect()->back()->with('success', $response['message']);
                } else {
                    return redirect()->route('adminSettings', ['tab' => 'others_api'])->withInput()->with('success', $response['message']);
                }
            }
        } catch(\Exception $e) {
            return redirect()->back()->with(['dismiss' => $e->getMessage()]);
    ...
    public function testMail(Request $request){
        ...
        $mailService->send('email.test_mail', [], $request->email, "Name", $subject);


        return redirect()->back()->with("success",__("Please Check Your Mail. If You Not Getting Mail Thats Mean Somthig Worng. Check Your Log File"));
    }

    public function userNavberSetting()
    {
        $data['navber'] = settings();
        return view('admin.settings.navber.navber',$data);
    }

    public function userNavberSettingSave(Request $request)
    {
        try{
            $response = $this->settingRepo->userNavberSettingSave($request->except('_token'));
            if ($response['success'])
                return redirect()->back()->with('success', $response['message']);
