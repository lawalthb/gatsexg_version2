...
//            $response = $service->sendNotificationProcess($request);
    ...
    public function clearEmailRecord()
    {
     ...
     // check node url
    public function adminCheckNodeUrl()
    {
        ...
        $result = $coinApi->checkWalletBalance($requestData);
        return $result;
    }
   ...
   *
   */
    public function adminCountryList(Request $request)
    {
        $data['title'] = __('Country List');
        $data['sub_menu'] = 'country';
        $data['items'] = CountryList::select('*')->orderBy('id','asc')->get();
    ...
    public function countryStatusChange(Request $request)
    {
        ...
        if(isset($request->active_id)) {
            $item = CountryList::find($request->active_id);
            if($item) {
