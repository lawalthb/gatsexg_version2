   ...
   */
    public function orderDisputeList(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $items = Order::join('order_disputes','order_disputes.order_id', '=','orders.id')
                ->where('orders.is_reported',STATUS_ACTIVE)
                ->where('order_disputes.status', '<>', STATUS_DELETED)
                ->select('orders.*','order_disputes.status as dispute_status');

            return datatables($items)
                ->addColumn('status', function ($item) {
                    return trade_dispute_status_web($item->dispute_status);
                })
                ->addColumn('coin_type', function ($item) {
                    return check_default_coin_type($item->coin_type);
                })
    ...
    public function orderDisputeDetails($id)
    {
        ...
        $data['admins'] = User::where(['status' => STATUS_ACTIVE])
    ...
    public function adminAssignDisputeAdmin(Request $request)
    {
        $response = $this->service->assignAdminToDispute($request);
        if ($response['success'] == true) {
            return redirect()->back()->with('success', $response['message']);
        } else {
    ...
    public function adminCancelDispute($id)
    {
        $response = $this->service->adminCancelDisputeProcess($id);
        if ($response['success'] == true) {
            return redirect()->route('orderDisputeList')->with('success', $response['message']);
        } else {
            return redirect()->back()->with('dismiss', $response['message']);
        }
    }
