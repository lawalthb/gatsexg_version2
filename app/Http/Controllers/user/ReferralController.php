...
class ReferralController extends Controller
{
    ...
    public function myReferral()
    {
        ...
        $data['referrals_3'] = DB::table('referral_users as ru1')->where('ru1.parent_id', Auth::user()->id)
            ...
            ->select('ru3.user_id as level_3', 'users.email','users.first_name as first_name', 'users.last_name as last_name','users.created_at as joining_date')
            ->get();
        $data['referrals_2'] = DB::table('referral_users as ru1')->where('ru1.parent_id', Auth::user()->id)
            ->Join('referral_users as ru2', 'ru2.parent_id', '=', 'ru1.user_id')
            ->join('users', 'users.id', '=', 'ru2.user_id')
            ->select('ru2.user_id as level_2','users.email','users.first_name as first_name', 'users.last_name as last_name','users.created_at as joining_date')
            ->get();
        $data['referrals_1'] = DB::table('referral_users as ru1')->where('ru1.parent_id', Auth::user()->id)
            ->join('users', 'users.id', '=', 'ru1.user_id')
            ->select('ru1.user_id as level_1','users.email','users.first_name as first_name', 'users.last_name as last_name','users.created_at as joining_date')
            ->get();
        $referralUsers = [];

        foreach ($data['referrals_1'] as $level1) {
            $referralUser['id'] = $level1->level_1;
            $referralUser['full_name'] = $level1->first_name.' '.$level1->last_name;
        ...
        foreach ($data['referrals_2'] as $level2) {
            ...
            $referralUser['full_name'] = $level2->first_name.' '.$level2->last_name;
        ...
        foreach ($data['referrals_3'] as $level3) {
            ...
            $referralUser['full_name'] = $level3->first_name.' '.$level3->last_name;
